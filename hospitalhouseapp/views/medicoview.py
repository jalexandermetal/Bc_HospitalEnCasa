...
from hospitalhouseapp.serializers.medicoserializers import Medicoserializers
...
class MedicoListCreateView(generics.ListCreateAPIView):
    ...
    def list(self,  request):
        ...
        serializer = Medicoserializers(queryset, many= True)
        return Response(serializer.data)

    def post(self, request, *args, **kwargs):
        print("post de medicos ")
        print(request.data)
        ...
        serializerEnf = Medicoserializers(Data=enfData)
        serializerEnf.is_valid(raise_exception=True)
        serializerEnf.save()
        return Response(status=status.HTTP_201_CREATED)


class MedicoRetrieveUpdateDeleteView(generics.RetrieveUpdateDestroyAPIView):
        queryset = Medico.objects.all()
        serializer_class = Medicoserializers
        lookup_field = "id"
        lookup_url_kwarg = "pk" 
